...
void sprite_init(Sprite *sp, Pointreel npr, SDL_Rect npi, int ang, Pointreel spee, Pointreel acc, SDL_Surface surface, int spriteta, int colork)
{
	(*sp).pr.rx=npr.rx;
	(*sp).pr.ry=npr.ry;
	(*sp).pi.x=npi.x;
...
void asteroid_init(Sprite *sp)
{
	...
	SDL_FreeSurface(temp);
	*/
	sprite_init(sp, npr, position, rand()%36, asteroidSpeed, asteroidAccel,*asteroidImage, taille, colorkey);
	return;
}
...
void fragmentationAsteroid(int id_cible)
{
	...
	if(nbAsteroid + nbFragment < ASTEROID_MAX)
	{
		...
		for (j = 0; j < nbFragment; j++)
		{
			...
			sprite_init(&new, asteroidTab[id_cible].pr, asteroidTab[id_cible].pi,0,asteroidSpeed, asteroidAccel,*asteroidImage, asteroidTab[id_cible].spritetaille/2, colorkey);
			asteroidTab[nbAsteroid] = new;
			nbAsteroid++;
		}
	}
...
void sprite_shoot_init(Sprite *sp1, Sprite *sp2){
	...
	sprite_init(sp1, posr, plop, (*sp2).angle, npr2, npr, *shootImage, bullet_size, colorkey);
...
void plusAnim(Sprite cible)
{
	if(nbAnim < ANIM_MAX)
	{
		...
		sprite_init(&animation, cible.pr, cible.pi, 0, nul, nul, *animImage, 32, colorkey);
...
// Fonction pour étendre le tableau de touche appuyées
...
void testColision(Sprite* sp){
	...
	if((nbAsteroid>0))
	{
		for(j=0; j<nbAsteroid; j++)
		{
			if(distanceEntreSprites(*sp,asteroidTab[j]) == 0)
			{
				//gameover = 1; 
			}
		}
