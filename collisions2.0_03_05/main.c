...
int main(int argc, char* argv[]){
	...
	*/
	...
	sprite_init(&vaisseau,npr,npi,ship_angle, speed_vaisseau, accel_vaisseau, *ship, ship_size, colorkey_vaisseau);
	...
	while(!gameover)
	{
		...
		// TEST NOUVELLES COLLISIONS
		for (i = 0; i<nbAsteroid; i++)
		{
			collide_test(&vaisseau,&asteroidTab[i],
			screen->format, &pu, &pv);
		}
