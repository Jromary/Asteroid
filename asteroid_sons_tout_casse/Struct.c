...
void fragmentationAsteroid(int id_cible)
{
	...
	if(nbAsteroid + nbFragment < ASTEROID_MAX)
	{
		...
		for (j = 0; j < nbFragment; j++)
		{
			...
			asteroidSpeed.ry = 0;
			//sprite_init(sp, npr, position, rand()%36, asteroidSpeed, asteroidAccel,*asteroidImage, taille);
			sprite_init(&new, asteroidTab[id_cible].pr, asteroidTab[id_cible].pi,0, asteroidTab[id_cible].speed, asteroidAccel,*asteroidImage, asteroidTab[id_cible].spritetaille/2);
			asteroidTab[nbAsteroid] = new;
			nbAsteroid++;
		}
		printf("----------------\n");
...
void splitAsteroid()
{
	...
	else
	{
		...
		// test de debug, parcours de tout les gros asteroid et split de ceux ci
		int k;
		for (k=0; k<nbAsteroid;k++)
		{
